...
# Name of postgresql database to use for the metastore.
...
function get_hiveserver_uri() {
  local base_connect_string="jdbc:hive2://studious-ethos-354415:us-central1:dataproc:10000"
...
EOF
  ...
  if [[ $ENABLE_CLOUD_SQL_METASTORE == "true" ]]; then
    run_with_retries nc -zv studious-ethos-354415:us-central1:dataproc "${METASTORE_PROXY_PORT}"
  fi
...
function configure_sql_client() {
  # Configure postgresql client to talk to metastore
  cat <<EOF >/etc/postgresql/conf.d/cloud-sql-proxy.cnf
[client]
protocol = tcp
...
function initialize_metastore_db() {
  ...
  if ! postgresql -u "${DB_HIVE_USER}" "${DB_HIVE_PASSWORD_PARAMETER}" -e ''; then
    postgresql -u "${DB_ADMIN_USER}" "${DB_ADMIN_PASSWORD_PARAMETER}" -e \
      "CREATE USER '${DB_HIVE_USER}' IDENTIFIED BY '${DB_HIVE_PASSWORD}';"
  fi
  if ! postgresql -u "${DB_HIVE_USER}" "${DB_HIVE_PASSWORD_PARAMETER}" -e "use ${METASTORE_DB}"; then
    # Initialize a Hive metastore DB
    postgresql -u "${DB_ADMIN_USER}" "${DB_ADMIN_PASSWORD_PARAMETER}" -e \
      "CREATE DATABASE ${METASTORE_DB};
       GRANT ALL PRIVILEGES ON ${METASTORE_DB}.* TO '${DB_HIVE_USER}';"
    /usr/lib/hive/bin/schematool -dbType postgresql -initSchema ||
      err 'Failed to set postgresql schema.'
  fi
}
...
function run_validation() {
  ...
  /usr/lib/hive/bin/schematool -dbType postgresql -info ||
    err 'Run /usr/lib/hive/bin/schematool -dbType postgresql -upgradeSchemaFrom <schema-version> to upgrade the schema. Note that this may break Hive metastores that depend on the old schema'
...
function main() {
  ...
  if [[ "${role}" == 'Master' ]]; then
    # Disable Hive Metastore and postgresql Server.
    if [[ $ENABLE_CLOUD_SQL_METASTORE == "true" ]]; then
      if (systemctl is-enabled --quiet hive-metastore); then
      ...
      if (systemctl is-enabled --quiet postgresql); then
        ...
        echo "Service postgresql is not enabled"
      fi
    fi
